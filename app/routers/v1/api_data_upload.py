...
from app.commons.data_providers import async_session_job_get_status, SrvRedisSingleton
...
from app.resources.error_handler import catch_internal, ECustomizedError, customized_error_template
from app.models.file_data import SrvFileDataMgr
from app.models.folder import FolderMgr, FolderNode, batch_create_4j_foldernodes, batch_link_folders
from app.models.fsm_file_upload import EState, FsmMgrUpload
from app.models.models_upload import (
    PreUploadResponse, PreUploadPOST, EUploadJobType, ChunkUploadResponse,
    OnSuccessUploadPOST,GETJobStatusResponse, POSTCombineChunksResponse
)
from app.models.tag import SrvTagsMgr
from app.resources.helpers import (
    update_file_operation_logs, get_geid, get_project, send_to_queue,
    async_delete_by_session_id, generate_archive_preview, async_get_geid
)
from app.resources.lock import async_lock_resource, async_unlock_resource, unlock_resource
...
class APIUpload:
    ...
    async def upload_pre(self, request_payload: PreUploadPOST, session_id=Header(None)):
        ...
        if request_payload.job_type == EUploadJobType.AS_FILE.name \
            ...
            for upload_data in request_payload.data:
                ...
                upload_data.resumable_filename = upload_data.dcm_id + "_" + upload_data.resumable_filename \
            ...
            if request_payload.job_type == EUploadJobType.AS_FILE.name:
                conflict_file_paths = await get_conflict_file_paths(
            ...
            for upload_data in request_payload.data:
                ...
                await run_in_threadpool(status_mgr.set_job_id, resumable_identifier)
    ...
    async def get_status(self, project_code,
        ...
        if not session_id:
            ...
            _res.error_msg = "Invalid Session ID: " + str(session_id)
            return _res.json_response()
        job_fatched = await async_session_job_get_status(
    ...
    async def clear_status(self, session_id: str = Header(None)):
        ...
        if not session_id:
            ...
            _res.error_msg = "Invalid Session ID: " + str(session_id)
            return _res.json_response()
        await async_delete_by_session_id(session_id, action=_JOB_TYPE)
        _res.code = EAPIResponseCode.success
        _res.result = {
            "message": "Success"
        }
    ...
    async def upload_chunks(self,
        ...
        # check pipeline id
        resumable_filename = dcm_id + "_" + resumable_filename \
            if dcm_id and dcm_id != "undefined" else resumable_filename
        temp_dir = await get_temp_dir(resumable_identifier)
        ...
        # init status manager
        status_mgr = await run_in_threadpool(
            FsmMgrUpload,
            session_id,
            project_code,
            _JOB_TYPE,
        ...
        try:
            ...
            destination = await run_in_threadpool(
                os.path.join, temp_dir, chunk_name
            )
            self.__logger.info(
    ...
    async def on_success(self, request_payload: OnSuccessUploadPOST,
        ...
        # check pipeline id
        request_payload.resumable_filename = request_payload.dcm_id + "_" + request_payload.resumable_filename \
        ...
        chunk_paths = [
            await run_in_threadpool(
                os.path.join, temp_dir, generate_chunk_name(request_payload.resumable_filename, x)
            )
    ...
    try:
        ...
        try:
            ...
            result = mc.client.fput_object(
                bucket, obj_path, temp_merged_file_full_path,
            )
        ...
        obj_path = (ConfigClass.GREEN_ZONE_LABEL if namespace == "greenroom" else ConfigClass.CORE_ZONE_LABEL) + "/" + obj_path
        # add upload logs
        ...
        payload = {
            ...
            "payload": {
                ...
                ConfigClass.DCM_PIPELINE_ID: request_payload.dcm_id,
